...
검증 erd_full.png: 라벨↔테이블 0 · 라벨↔라벨 0 · 선↔테이블 0 · 세로선 중첩 0 · 가로선 중첩 3(허용)
```

허용된 항목의 0 아닌 값은 `(허용)` 이 붙어 나온다. 0 이어야 하는 항목이 0 이 아니면
같은 줄 끝에 `[경고] 0 이어야 한다: …` 가 붙는다 — **`[경고]` 가 보이면 회귀다.**
...
- **가로선 중첩** — 같은 컬럼으로 모이는 합류는 제외하고 센다. 영역별 상세도는 0이
  나오는 것이 보통이고, 전체도·개요도는 노드 진출 y가 고정이라 소수 남을 수 있다 —
  그 둘에서만 `(허용)` 으로 표시된다.
  한 영역에서 계속 0이 안 되면 한 컬럼 행으로 너무 많은 선이 모이는 것이다 —
  `hgap` 을 넓히거나 spec 에서 그 영역을 쪼갠다.
