...
| `ERD_MAX_AREAS` | `12` | 자동 분류 시 영역 수 상한 — 문서 전체 기준이며 '기타' 영역도 이 안에 든다. 스키마를 버리지는 않으므로 스키마가 상한보다 많으면 스키마마다 한 영역씩은 나온다. |
...
는 한동안 개요도·전체도에만 `tolerate=('h_overlap',)` 을 주고 영역도에는 주지 않아,
똑같은 겹침이 한쪽에서는 경고하고 두쪽에서는 조용했다. 그 봐주기를 넣은 이유 —
라우터가 y 를 옮길 수 없는 진출입 꼬리 — 는 지금 **세는 쪽 안에서** 처리한다(아예
세지 않는다). 한 번 더 잠재우는 것은 라우터가 **고칠 수 있는** 나머지를 가릴 뿐이다.
...
```bash
python3 selftest.py            # 전부 (101개)
python3 selftest.py parse      # 이름에 'parse' 가 든 항목만
python3 selftest_history.py    # 그 파일 것 39개만 (10초쯤)
```
