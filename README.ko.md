...
라벨이 테이블을 덮거나 선이 겹치면 숫자로 드러난다. 모든 항목이 0 이어야 하고, 0 이어야
하는 값이 0 이 아니면 같은 줄 끝에 `[경고]` 가 붙는다. `해당 없음` 은 그 그림에 그
검사가 해당하지 않는다는 뜻이다 — 개요도는 관계 라벨을 그리지 않으므로 라벨 항목은 잴
것이 없고, 여기에 0 을 찍으면 하지도 않은 검사가 깨끗하다고 말하는 셈이 된다.
...
- `psql` 또는 `docker` 중 하나
- 서버는 PostgreSQL **9.4 이상** (9.4·9.6·10·11·12·16·17 에서 확인). 그보다 낮으면
  반쪽 스키마를 만들지 않고 메시지를 내고 멈춘다
...
**④·⑤ 는 `schema.json` 보다 오래된 그림 위에서는 돌지 않는다.** 표와 그림이 서로 다른
스키마를 말하는 문서를 내주는 대신 `그림 N장이 … 보다 오래됐다` 로 멈춘다 — ③ 을 다시
돌리면 된다. 문구만 고쳐 그림이 정말 그대로여도 되는 경우엔 `ERD_STALE=warn` 이 그대로
넣고 매번 그 사실을 찍는다(빈 값 `ERD_STALE=` 은 여기 다른 스위치들과 같이 **꺼짐**이다).

**설정 파일 없이도 돌아간다.** 스키마와 테이블명 접두어로 영역을 자동 분류하고 색을
배정한다.

다만 자동 분류는 **일단 그려보는 용도**다. 이름 규칙이 고른 DB가 아니면 어디에도 안
...
아직 반영하지 않은 변경까지 문서에 넣으려면 `introspect.py` 대신 `parse_ddl.py` 를
돌린다 — `$ERD_SQL_DIR/*.sql`(기본값 `$ERD_PROJ/sql`)을 읽어 같은 `schema.json` 을 쓴다.

그 DDL 의 `--` 주석이 그대로 설명이 된다. **주석이 어디에 놓였는지가 누구의 설명인지를
정한다:**

```sql
-- 주문 한 건에 한 행           ← CREATE TABLE 위 → 테이블 설명
CREATE TABLE orders (          -- 주문 한 건에 한 행  ← 이 줄 → 테이블 설명 (이쪽이 이긴다)
  id       bigint PRIMARY KEY, -- 행 id              ← 쉼표 뒤 → 이 컬럼
  -- 주문한 사람               ← 컬럼 위 → 그 컬럼 (여러 줄이면 이어 붙인다)
  user_id  bigint NOT NULL,
  code     text,
  -- 테넌트마다 유일           ← 제약 위 → 아무의 것도 아니다
  UNIQUE (user_id, code)
  -- TODO: 통화 컬럼 추가      ← 마지막 컬럼 뒤 → 아무의 것도 아니다
);
```

`/* … */` 블록은 설명이 되지 않는다. 그 안에 든 `--` 줄도 마찬가지다. `COMMENT ON
TABLE/COLUMN` 은 위의 것을 전부 덮어쓴다 — 그래서 `pg_dump` 파일과 손으로 쓴 파일이
같은 결과를 낸다.
...
같은 배치를 두 번 그린 것이다. 테이블 자리와 상자 크기는 한 번의 배치 계산에서 나오므로
두 그림에서 테이블은 같은 자리에 놓이고, 그리기만 벡터로 바뀐다(`svg_canvas.py`가
`ImageDraw` 인터페이스를 흉내 낸다). 다만 SVG 쪽은 다른 배율로 도는데 PIL 의 글자 폭은
배율에 비례하지 않는다 — 그래서 관계 라벨이 SVG 에서는 다른 자리에 앉을 수 있다(무작위
스키마 20벌에서 라벨 하나가 134px 옮겨 앉았다). **판마다 찍히는 검증 숫자는 PNG 를
잰 것이다.**
